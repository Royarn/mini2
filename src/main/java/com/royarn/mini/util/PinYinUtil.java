 ...
 *
 */
package com.netposa.npconfig.support;
...
public class PinYinUtil {
    ...
    public static PinYinMsg getPinYinString(String chineseCharString) {
        if (chineseCharString != null
                && !chineseCharString.trim().equalsIgnoreCase("")) {
            char[] srcChar;
...
//						if (pinYinChar[0].matches("[a-zA-Z]+"))
    ...
    public static String spellFirstUpper(String chinese) {
        String spellFirst = spellFirst(chinese);
        if (!StringUtils.isEmpty(spellFirst)) {
            return spellFirst.toUpperCase();
        }
        return "";
    }

    public static String spellFirst(String chinese) {
        if (chinese == null) {
            return "";
        }
        String pinyin = null;
        ...
        try {
            ...
            for (int i = 0; i < chinese.length(); i++) {
                String[] array = PinyinHelper.toHanyuPinyinStringArray(chinese.charAt(i), format);
                if (array == null || array.length == 0) {
                    pinyin = String.valueOf(chinese.charAt(i));
                } else {
                    String s = array[0];// 不管多音字,只取第一个
                    char c = s.charAt(0);// 大写第一个字母
        ...
        if (len >= 2) {
            ...
            for (int index = 0; index < len1; index++) {
                for (int j = 0; j < len2; j++) {
                    temp[Index] = strJaggedArray[0][index]
                            + strJaggedArray[1][j];
                    Index++;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(str + "::" + spellFirstUpper(str));
        System.out.println(str + "::" + spellFirst(str));
		/*System.out.println(str+"::"+spellAllFirstUpper(str));
		System.out.println(str+"::"+getPinYinSet(str));*/
        System.out.println(str + "::" + getPinYinString(str).pinyin);
    }
